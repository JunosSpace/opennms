...
public class NMACommand {
	...
	public NMAResponse execute(String method) throws IOException {
		...
		try {
			...
			// logger.debug("Fething timeoutvalue");
			int defualtnmacommandtimeout = ServerConfiguration.getInt(
					SpaceTypedConfigAttributes.DEFAULT_NMA_TIMEOUT_ATTR, 120);
			// logger.debug("NMA Command timeout is configured to "
			// + defualtnmacommandtimeout + " minutes");
			conn.setConnectTimeout(defualtnmacommandtimeout * 60 * 1000);
			conn.setReadTimeout(defualtnmacommandtimeout * 60 * 1000);
